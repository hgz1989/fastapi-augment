...
@Description    : 发现并加载 apps 包下通过 __all__ 导出的 ASGI 应用
约定：
    每个业务子包（如 apps.platform）在其 __init__.py 中通过 __all__ 导出
    自己创建的 ASGI 应用实例（如 platform_app）。只有出现在子包
    __init__.py 的 __all__ 里、且确实是可调用 ASGI 应用的对象才会被
    识别为"应用"，其它一律不算（判定见 ``common.asgi_types.is_asgi_app``）。
    主应用由配置自行定义（settings.project.main_app，如
    ``apps.platform:platform_app``），不参与自动发现；
    discover_asgi_apps(exclude=主应用) 用于获取主应用之外的其它应用。
"""
...
from .asgi_types import ASGIApplication, is_asgi_app
...
def _iter_exported_apps(module: ModuleType) -> Iterator[tuple[str, ASGIApplication]]:
    """从模块的 __all__ 中筛出可调用的 ASGI 应用

    Args:
        module: 子包模块（即其 __init__.py 对应的模块对象）

    Yields:
        (导出名, ASGI 应用)；未定义 __all__ 或没有 ASGI 应用时为空
    """
...
def discover_asgi_apps(root: str | Path = 'apps', exclude: str | None = None) -> list[ASGIAppSpec]:
    """发现 root 包下所有子包通过 __all__ 导出的 ASGI 应用

    规则：
    1. 递归遍历 root 下的全部子包（如 ``apps.platform``）
    2. 只检查子包 ``__init__.py`` 中 ``__all__`` 列出的名字
    3. 名字对应的对象必须是可调用的 ASGI 应用才计入，其它对象一律不算
    4. 结果按模块名排序，保证顺序稳定
    ...
    for module_name in sorted(_iter_subpackages(root)):
        ...
        for name, app in _iter_exported_apps(module):
            specs.append(ASGIAppSpec(module=module_name, name=name, app=app))
    if exclude:
